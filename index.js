...
drawglobe("/test?tmstp=" + x, 'o', x);


function drawglobe(url1, ow, tmstp) {
//alert(url1);
...
_yr = tmstp.substring(0,4); 
_m = tmstp.substring(5,7); 
_d = tmstp.substring(8,10); 
_hh = tmstp.substring(11,13);
_mm = tmstp.substring(14,17);
//alert(new Date(_m+'-'+_d+'-'+_yr+'-'+_hh+':'+_mm+'Z'));
view.environment.lighting.date = new Date(_m+'-'+_d+'-'+_yr+'-'+_hh+':'+_mm+'Z').getTime();
//alert(tmstp);
            ...
            document.getElementById("basemap_settings").innerHTML = "";
            searchWidget = "";
...
function foo2() {
    ...
    var tmp =  moment(date+'-'+time,'YYYY-MM-DD HH:mm').subtract(5, 'minutes');
    date = tmp.format('YYYY-MM-DD');
    time = tmp.format('HH:mm');        
    }
    return date+'-'+time;    
}
function refresh() {
    document.getElementById("loader1").style.visibility = 'visible';
    //var xx = foo2();// alert(xx);
    var xx = foo2();
   // alert(xx);
    ...
    if ($('#ow').prop('checked') != true) {
        ...
        drawglobe("/weimer?tmstp=" + xx, 'w', xx);
    } else {
        drawglobe("/test?tmstp=" + xx, 'o', xx);
    }
